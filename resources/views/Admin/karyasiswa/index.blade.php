    ...
    <div class="w-full px-3">
        <div class="relative flex flex-col min-w-0 mb-4 break-words bg-white shadow-soft-xl rounded-2xl">
            <div class="p-4 flex justify-between items-center">
                <h5 class="font-bold">Karya Siswa</h5>
                <a href="{{ route('admin.karyasiswa.create') }}" class="btn-admin">Tambah Karya</a>
            </div>
        ...
        <tr>
            ...
            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
              <span class="text-xs font-semibold leading-tight">{{ $item->kategori ?? '-' }}</span>
            </td>
            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
              @if($item->image)
                <img src="{{ asset('storage/'.$item->image) }}" class="h-12 w-12 rounded-xl" alt="{{ $item->nama }}" />
              @else
                <span class="text-xs text-slate-400">Tidak ada</span>
              @endif
            </td>
            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
              <a href="{{ route('admin.karyasiswa.edit', $item) }}" class="btn-admin-secondary">Edit</a>
              <form action="{{ route('admin.karyasiswa.destroy', $item) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Yakin ingin dihapus?')">
                    @csrf
                    @method('DELETE')
